...
namespace MediaBrowser.Providers.Plugins.Tmdb
{
    ...
    public class TmdbClientManager : IDisposable
    {
        ...
        // Trending shifts slowly and is identical for every user, so it is cached far longer
        // than per-item metadata to keep load off the shared API key.
        private const int TrendingCacheDurationInHours = 6;

        /// <summary>
        /// How many pages of TMDb results each list considers. TMDb returns 20 per page, and most
        /// of them will not be in any given library, so a wide net is cast to land a usable number
        /// of hits.
        /// </summary>
        public const int DiscoverPagesToScan = 15;
        ...
        /// </summary>
        /// <param name="cacheKey">The cache key for this list.</param>
        /// <param name="fetchPage">Fetches the ids on a single one-based page, or null when exhausted.</param>
        /// <param name="pages">The number of pages to fetch.</param>
        /// <returns>The TMDb ids in the order the endpoint returned them.</returns>
            ...
            int pages)
        {
            ...
            // Ranking depends on order, so results are consumed in page order rather than
            // completion order.
            ...
            if (ids.Count > 0)
            {
                _memoryCache.Set(cacheKey, (IReadOnlyList<int>)ids, TimeSpan.FromHours(TrendingCacheDurationInHours));
            }
        ...
        /// <summary>
        /// Populates the cache for every discover list, so that the first user to open one does
        /// not have to wait for TMDb.
        /// </summary>
        /// <param name="cancellationToken">The cancellation token.</param>
        ...
        {
            await GetTrendingMovieIdsAsync(TimeWindow.Week, DiscoverPagesToScan, cancellationToken).ConfigureAwait(false);
            await GetTrendingSeriesIdsAsync(TimeWindow.Week, DiscoverPagesToScan, cancellationToken).ConfigureAwait(false);
            await GetTopRatedMovieIdsAsync(DiscoverPagesToScan, cancellationToken).ConfigureAwait(false);
            await GetTopRatedSeriesIdsAsync(DiscoverPagesToScan, cancellationToken).ConfigureAwait(false);
        }
